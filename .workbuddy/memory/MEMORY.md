...
## 时间尺度：前台压缩历 ↔ 后台真实历（批次九十八，改经济数值前必读）
- **`GameClock.Day` 是「旬」**：1 旬 = 24 游戏时 = **60 秒现实**；1 月 = 3 旬、1 年 = 36 旬 = 36 分钟。
  ⚠️ 曾把它当成"1 天"并误算成 20 秒现实，导致经济尺度整体差 10 倍——**这是本项目最容易踩的时间坑**。
- **两历映射**：`TimeConfig.RealDaysPerGameDay = 10`，即 **1 游戏旬 ≙ 10 真实日** ⇒ 1 游戏月 ≙ 30 真实日 = **1 真实月**、1 游戏年 ≙ 360 真实日 = **1 真实年**。**月与年天然对齐，只有"日"需要换算**——换算层存在的全部理由。
- **配置语义（写新数值时照此办）**：
  - **「日值」一律是"每真实日"**（如一人一天一升米 = `FoodPerDay = 1`），结算处经 `TimeConfig.PerRealDay` 换算成每旬；
  - **「月值」与真实月同义**（月薪、维护费、老化等），按 `DaysPerMonth` 逐旬摊即可（所有 `/ GameClock.DaysPerMonth` 的写法都是这个模式）。
- **改前台节奏只改一个数**：比如一年从 36 分钟拉到 60 分钟，只改 `RealDaysPerGameDay` 或 `SecondsPerGameHour`，经济数值一行不动。
- 已接入换算的点：`GoodsSystem`（口粮/柴薪/饮水/分级需求）、`CraftingSystem`（加工产能）、`EconomySystem`（铸币）。
  **未接入**（暂标待办）：`SkillExpPerDay`、`StartupChancePerDay` 等概率类（概率换算要用 `TimeConfig.ChancePerTick`）。

## 经济循环（实现版摘要，权威文档 = specs/ECONOMY.md）
- 钱在「外部注入 → 官库 → 村民家庭 → 铺面/工坊 → 回流官库」闭环内转；货在「自然 → 原料 → 中间品 → 成品 → 消费」单向流。
- **三条守恒律**（历批次反复修的坑）：① 官库该付的钱一律「先发放、按实扣款」(`GameState.PayBuildWages`，无人领则留官库)；② 家庭该交的钱一律实扣公产、不足少收 (`TakeFromFamily`/`TakeLandTax`)；③ **价只有一个出口**——只取 `Goods.PriceOf`(基价) / `Goods.RetailPrice`(基价×2.0×库存倍率×鲜度折价，家庭零买) / `Goods.BuyerPrice`(基价×库存倍率×鲜度折价，买方为铺面/工坊)，任何调用点不得自己乘倍率（各写各的正是「同货不同价」的成因）。
- **物价锚定（批次九十六重定）**：**一升米 = 10 文 = 一人一天用量**（`FoodPerDay = 1.0` 份/人/日），铺面零售 20 文上下（`BuyMarkup = 2.0`）。其余货品一律按「**成品价 ≈ 原料价 + 燃料价 + 约 22 文加工附加值**」推导——这样各条加工链的每工日毛利自动落在同一量级（35~56 文），不会再出现"做烧饼 4 文、做成衣 205 文"的 55 倍悬殊。**改任何价格前先看这条锚，别凭感觉定价。**
- **鲜度折价（批次九十六）**：`Goods.FreshPriceFactor`（下限 `FreshPriceFloor = 0.5`）接入两个定价出口；加工侧 `CraftingSystem` 的可产数与扣料也改走有效量。**这两处堵的是"把时效风险无损转嫁给 NPC"的漏洞**——此前铺面按基价照收、加工不看鲜度，玩家可把快失效的货卖给铺面或放陈后加工，整套时效机制会形同虚设。
- **家计 30 文/人/月**（批次九十六从 200 降下来）：旧值是无实物的隐形人头税（真实生活成本 30 文的 6.7 倍，且与买粮重复），且是官库最大收入（土地税的 20 倍）。官库主体收入现由土地税接管——`TaxSystem.BuildingTaxBase` 已同步放大 6~10 倍（民居 10→60、商铺 50→300）。
- **库存联动定价（批次九十四已接线）**：`Goods.FactorOf` = `StockPriceFactor(整仓占用率 Inv.Total/Inv.Capacity)`；档位 ≥95% ×0.7 / ≥80% ×0.9 / ≤20% ×1.1。容量≤0 的非贸易建筑取平价（否则 0% 占用被误判缺货加价）。
- **两个曾经的数值错误（批次九十六修正，别再犯）**：① 铸币局 `mintPerWorkerDay` 50 → **800**（旧值下 600 文/月收入对 4800 文/月工资，是确定的亏本项目）；② 农田 `yieldPerWorker` 50→150、`salary` 800→**400**（旧值下农夫月产粮值 167 文却领 800 文工资，官库补贴 4.8 倍）。
...
- **稳定期（批次九十七）**：`TimedBaseline.PlateauDays`（单位是**露天基准日**，仓房实际 = ÷0.45）。**鲜度不由逐日扣减得到，而由 `FreshAgeDays` 派生**（`TimedRules.FreshOf`）：稳定期内恒为 100，之后才线性下滑。粮食 `(112, 480, 48)` → 仓房 **2.96 年转入打折 / 6.32 年不可食**、露天 1.33 / 2.84 年。各货品稳定期：腌 30、酒 90、木炭 120、木板/木器 150、家具 240、皮革 360、铁锭 480、盐/铁器 720；生鲜与活物一律 0。
  ⚠️ **调时效数值时记住：「多久完全不变」与「之后坏多快」是两个独立旋钮**——只调后者，所有物品都会退化成"必须天天处理的易耗品"，长期仓储策略失去存在空间。
