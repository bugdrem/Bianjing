...
「先宏观后细节」的生成模型（批次四十九起；批次九十三重构山水）：
1. **草图规划**（`WorldSketch`，128² 内存小图，映射比 8m/格）：西北高东南低对角趋势（`TrendHeight`）+ 平原低频 fBm；**主峰 2 座（88~105m，`PrimaryPeakCount`）**以角向谐波高斯锥（3 阶 + 2 阶方位调制，破完美圆锥）撒在西北半包围带并按 `PrimaryPeakOverbuild 6m` 超填；普通峰 `PeakCount 10~14` 座 30~62m 高斯锥；山脊连接最近邻峰（余弦横截面 + 鞍部下凹）；**低矮小山 12~20 座成簇（2~4 座/簇）撒布**，选址判据 = 峰群影响强度 `PeakInfluence < 0.22`（远离既有峰群即可，东北/西南/中部空档皆可落），两道守卫：落点坡角 ≤16°、高宽比 ≤0.40（矮胖不尖刺）。
2. **上采样**映射到 1025² 顶点。
...
5. **主峰二次隆升**（`ReapplyPrimaryPeaks`）：侵蚀会削顶，按草图登记的目标高斯面余弦羽化补回（只抬不压）；随后统一 clamp 到 `[MinTerrainHeight -5m, MaxTerrainHeight 110m]`。
6. 草图另跑一份低分辨率 `FlowRouter`+`RiverNetwork` 产出**预览河网**（`PreviewRivers`，随机地图菜单示意线，与成品水系同源算法）。
...
### 4.5 水系（FlowRouter + RiverNetwork + LakeGenerator + RiverGenerator）

水系完全在**侵蚀完成的成品地形**上按真实汇流求解（批次九十三，脱离草图定线）：

1. **汇流求解**（`FlowRouter`，路由格 2m）：Priority-Flood 填洼（图缘起按填充高升序出队，`Filled = max(本格, 来路+ε)`）→ 出队序即拓扑序 → D8 流向（指向使其出队的邻格）→ 逆序单趟汇流累积 `Area`；`Depression = Filled − Base` 即天然洼地深度。
2. **河网**（`RiverNetwork`）：汇流面积 ≥`ChannelMinDrainCells 45`（阈值逐级折半放宽保底）且无达标上游的格为源（≤`ChannelMaxCount 13` 条）；顺 D8 走线；河宽 = `WidthCoef 0.075 × √汇流面积`（Hack 定律，钳 5~34m，滑动平均）；**Chaikin 圆角 ×2** 消 D8 阶梯；**蛇曲**沿弧长正弦摆动（振幅 = 河宽×1.5、波长 210m）+ 垂向最低点吸附（`MeanderSnap 0.5`）贴回谷底；**河口外推**：末端伸到图外 `MouthExtendMeters 56m`、末段 170m 内渐展到 2.1 倍宽——治「出图是小点」。
3. **湖泊**（`LakeGenerator`）：① 天然洼地塘——Depression 连通块（填深 ≥0.45m）按溢出高程漫灌；② 三档选址湖——大 70~110m / 中 32~58m / 小 12~26m，拒绝采样（避图缘/城心/湖间 ≥170m、局部低洼平坦）后以「最低前沿洪水填充」生长到目标面积（形状贴地形非正圆，闸门：最大水深 3.2m / 最大伸展 2.4×）；共 **3~6 座**、总水面 ≤5.5%。
4. **刻盘**（`RiverGenerator.BuildWaterSystem`）：河道按弧长盖盘（格中心距判定）；**水位拓扑单趟求解**——按洪泛序先下游后上游取 max，水位沿流向单调不增，支流汇入自动回灌抬平；湖面水位冻结（同湖统一）；**河床下压**按连通水体最大离岸距离在 1.0~3.5m 间插值（窄河浅槽、大湖深盆），离岸 `BedFalloffRatio 0.35` 渐浅，钳到 `MinTerrainHeight+0.05`。
5. 逐格 `WaterH` 存储水位，渲染时顶点取邻水格均值，坡河上水面连续倾斜。
...
## 5. 渲染系统（GridRenderer 协调器 + 六图层）

### 5.1 图层拆分与分块增量渲染（批次九十三）

地表内容拆成六个独立 Node3D 图层（`scripts/map/layers/`，基类 `MapLayer`）：
- **TerrainLayer**：每分块 65×65 顶点三角网格；顶点色四段（河床泥沙/平原橄榄混色/岩褐/雪白 ≥68m）。
- **WaterLayer**：每分块半透三角网格（顶点插值水位，透见河床，不投影）。
- **RoadLayer**：三类砖纹路面（世界坐标 UV 全图连续）+ 桥面/引桥实体板，各一张网格。
- **VegetationLayer**：树木按分块 MultiMesh（原始体三件套 + 每树种外部模型层）。
- **BuildingLayer**：建筑 13 个角色 MultiMesh（整层重建）+ glb 资产挂载点 `_assetRoot`。
- **OverlayLayer**：坊区色块 + 建造网格线（平时隐藏，显隐切换不牵动实体层）。

`GridRenderer` 瘦身为**协调器**：订阅 EventBus 翻译脏标记、`_Process` 预算仲裁、持有图缘裙板（属 Scroll 卷轴层）；
分块几何由 `ChunkGeometryBuilder` **单趟遍历**产出（水面/三路/桥/树全组缓冲，免各层各扫一遍），分发给各图层 `ApplyChunk`。

地图按 **64×64 分块**组织，每块独立 `ArrayMesh`。脏标机制订阅 EventBus：
- `MapChanged` → 全部分块脏。
- `CellChanged` → 所在分块脏（块缘格连带邻块防接缝错位）。
- `RectChanged` → 覆盖矩形的分块脏。
- `BuildingsChanged` → 只重建建筑层（`BuildingLayer.Rebuild`）。
- `TreesChanged` → 只刷新各分块树木 MultiMesh（`TreesDirty`，只填 MultiMesh 不重建网格）。

每帧限量重建（约 12 块整块 + 32 块树层），避免大改动时一次性百万格重建卡顿（4x 频繁建房曾是间歇卡顿主源，经事件细分 + 分块限额解决）。**预算仲裁集中在协调器**——图层若各跑一份 `_Process` 预算会把同帧重建量翻六倍。
...
| `TerrainConfig` | 高度场生成与坡度规则 | 步高 0.5m、坡度 30°、垫基高差 1m；海拔 [-5, 110]m；草图 128² 映射 8m/格；大势 6m；主峰 2 座 88~105m（半径 160~210m，超填 6m）；峰 10~14 座 30~62m（半径 60~120）；低矮小山 12~20 座 2~14m 成簇（峰群影响 <0.22 处可落、坡角 ≤16°、高宽比 ≤0.40）；山区带 440m、避心圆 280m；山脊 2 链/鞍部 0.62/半宽 46m；侵蚀 25 万滴 + 6 千滴；热侵蚀安息角 ≈33°；采集豁免 4.5m |
| `WaterConfig` | 河网 / 湖泊 / 河床（汇流求解制，批次九十三） | 路由格 2m（`RouteDownsample`）、填洼 ε 0.0005；河道 ≥45 汇流格、≤13 条、≥55 路由格长；河宽 0.075×√汇流面积（5~34m、平滑窗 9）；Chaikin ×2、蛇曲振幅 1.5×河宽/波长 210m/吸附 0.5；河口外推 56m、展宽 2.1×、渐宽段 170m；水面 = 沿程地形 −0.35m（下限 0）；湖 3~6 座（大 70~110 / 中 32~58 / 小 12~26m）、洼地塘填深 ≥0.45m、总水面 ≤5.5%、湖间 ≥170m；河床深 1.0~3.5m（离岸距离插值、falloff 0.35×最大离岸）；水面外扩 0.7m（仅朝非水方向） |
...
- **glb 建筑资产管线**（`scripts/build/BuildingAssetLoader.cs` + `BuildingDef.ModelPath`/`HasModel`）：
  ...
  - `BuildingLayer.Rebuild` 优先加载 glb，失败自动回落 `BuildingModelFactory` 原语；`CleanupStaleAssetInstances` 回收过期实例。
