...
## 批次九十八（2026-09-17）前台压缩历 ↔ 后台真实历：时间换算层 + 经济尺度对齐史料

需求：在后台引入真实历法（1 年 = 12 月、1 月 = 30 日），前台与后台做映射，日后改前台节奏也能快速对齐；并据此核对「宋代普通工人月薪 2000~7000 文、月米钱 300 文」。

- **订正一处时间尺度错误**：此前把"1 游戏日"当成 1 天（还误算成 20 秒现实）。实际 `GameClock.Day` 是**旬**——1 旬 = 24 游戏时 = **60 秒现实**，1 月 = 3 旬、1 年 = 36 旬 = 36 分钟。时效系统以"游戏日/游戏年"为单位的数值本身无误，但"1 日 = 20 秒"的描述错了，已全面订正。
- **两历映射（本批核心）**：新增 `TimeConfig.RealDaysPerGameDay = 10` 等换算层。**1 游戏旬 ≙ 10 真实日** ⇒ 1 游戏月 ≙ 30 真实日 = **1 个真实月**、1 游戏年 ≙ 360 真实日 = **1 个真实年**——**月与年天然对齐，只有"日"需要换算**，这就是换算层存在的全部理由。
- **配置语义随之下定**：「日值」一律是"每<b>真实日</b>"（如一人一天一升米），结算（按旬）时经 `TimeConfig.PerRealDay` 换算；「月值」与真实月同义，按 `DaysPerMonth` 逐旬摊即可。**日后改前台节奏（如一年从 36 分钟拉到 60 分钟）只改这一个系数，经济数值一行不动。**
- **接入点**：`GoodsSystem` 的口粮/柴薪/饮水与分级需求、`CraftingSystem` 的加工产能、`EconomySystem` 的铸币——全部改走 `TimeConfig.PerRealDay`。
- **尺度重定**（对齐史料）：月米钱 **300 文**（1 升/真实日 × 30 日 × 10 文）；工资 **2000~7000 文/真实月**（农田 2000 → 宫殿 7000）；王爷月俸 8 000 → **60 000**；家计 30 → **300**；官粮用度 0.05 → 0.5、朝廷粮饷 3 → 30、官仓上限 0.9 → 3；`LoadUnits` 5 → **30**（一担 = 一月口粮，且 `WoodPerHp = LoadUnits / ChopDamage` 自动保持"一斧恰好一担"）；`PileCapacity` 40 → 240；修缮料钱与摊派 ×10；耕牛价 10 000 → 100 000；建筑造价与 upkeep ×10；储物容量 ×10；农田 `yieldPerWorker` 150 → **1500**；铸币 `mintPerWorkerDay` 800 → 150（新尺度下 4 工 × 150 × 30 日 = 18 000 文/月，对工资 12 000，净赚 6 000）。
- **核对结果**：薪米比 **6.7~23.3 倍**，与史料区间（2000~7000 对 300）完全吻合；三口之家（双职工）月支出 3 297 文 / 收入 7 000 文 = **负担率 47%**（此前按错误尺度只有 7%，现在才接近农业社会的真实比例）。
- 影响文件：`configs/{TimeConfig,EconomyConfig}.cs`、`data/buildings.json`、`policy/TaxSystem.cs`、`sim/{GoodsSystem,CraftingSystem,EconomySystem}.cs`。

---

## 批次九十七（2026-09-17）时效系统补「稳定期」：粮食 2.96 年打折、6.32 年不可食

需求：核对「未经加工的稻谷仓库能存放 3 年不变质，超过 5 年才不能食用，之后可做饲料或其他」是否符合。

**核对结论：不符合**（两个指标都对不上，且根因不在数值大小）：

- 改造前鲜度是**从入库第一天就线性下降**，仓房内第 3 年已掉到 59%（勉强擦着全效果线下沿），"不可食用"（鲜度 15）则推迟到 **6.30 年**。
- 根因是**衰减曲线的形状**。谷物真实的储存曲线是"先长期稳定 → 之后加速劣变"，而线性衰减把"三年不变质"做成了"三年里慢慢掉到六成"——**两者可以把数值调得一模一样，对玩家的意义却完全不同**：前者意味着"可以放心囤三年"，后者意味着"要天天盯着"。

- **补「稳定期（平台期）」机制**：`TimedBaseline` 新增 `PlateauDays`（单位为露天基准日）。鲜度**不再由逐日扣减得到，而由累积老化龄期派生**——`TimedState` 新增 `FreshAgeDays`（累积老化龄期，受环境影响：仓房 ×0.45、露天 ×1.0），`TimedRules.FreshOf` 据此算鲜度：**稳定期内恒为 100，之后才线性下滑**。`Fresh` 保留为派生缓存（面板、定价、翻新照读，不必各自重算）。
- **粮食基线更新**：`FreshDays` 120 → **112**、新增 `PlateauDays = 48`。代入仓房环境（速率 ×0.45）后：稳定期（完全不变质）**2.96 年**、转入打折同为 2.96 年、不可食用（鲜度 15）**6.32 年**、鲜度归零 6.91 年、硬轴彻底消亡 21.3 年。露天条件下依次为 1.33 / 2.84 / 3.11 年——露天堆放本就存不住，这正是仓储的价值所在。
  （数值取舍：最初按"3 年不变质 / 5 年不可食"配置，后改采核算出的 2.96 / 6.32 —— 2.96 年**转入打折**比"3 年完全不变"更有张力；6.32 年才不能吃也比 5 年更从容。）
- **其他货品按耐储性配稳定期**：腌货 30、酒 90、木炭 120、木板/木器 150、家具 240、皮革 360、铁锭 480、精盐/铁器/兵刃 720；**生鲜与活物一律 0**（野味、烧饼、果品、生皮、药材、酒曲、柴薪——一离火离枝便开始劣变）。
- **翻新动作适配**：恢复 = 把 `FreshAgeDays` 回退到"目标鲜度"对应的龄期（`TimedRules.AgeForFresh`），只减不增（已比目标更新则不动）。
- 影响文件：`configs/TimelinessConfig.cs`、`sim/timeliness/{TimedState,TimedRules,TimelinessSystem,RenewAction}.cs`、`sim/Inventory.cs`（`GoodsStack` 加 `FreshAgeDays`，默认 0 = 全新，旧档安全）。

---

## 批次九十六（2026-09-17）物价重锚（一升米 10 文）+ 经济审计四条修复

需求：以「一升米 = 10 文 = 一人一天用量」重锚全物价，铺面零售 20 文上下并随外部环境浮动，其余按此推导；结合该锚修掉经济审计列出的 P0 + P1 四条。

- **物价重锚**：`Goods.BasePrice` 全表重定。米 **10 文**为锚，`BuyMarkup` 1.5→**2.0**（铺面零售 20 文上下，再按库存占用率与鲜度浮动）。其余货品按「**成品价 ≈ 原料价 + 燃料价 + 约 22 文加工附加值**」推导，各条加工链的每工日毛利从原来的 4~205 文（**55 倍悬殊**）收敛到 35~56 文——民生加工线（烧饼、木炭）由此第一次值得做。
- **口粮口径对齐锚定**：`FoodPerDay` 0.2333→**1.0**（一升一天）、`FuelPerDay` 0.07→0.3、`WaterPerDay`→1.0。旧值是"每旬"口径的残留，比"一升一天"少算约 4 倍。
- **家计 200→30 文/人/月**（P0）：旧值的问题不在"收"而在**名实不符且量级离谱**——家庭口粮本就单独付货款给铺面，这笔"柴米官营"等于收第二遍钱；200 文是真实生活成本（3 日 × 1 升 × 10 文 = 30 文）的 6.7 倍，且事实上是官库最大收入来源（远超土地税），让无业者永远无法自立。新值对齐"一人一月口粮"，定位为官府专营摊派。
- **土地税税基放大 6~10 倍**（配套，否则官库立刻赤字）：`TaxSystem.BuildingTaxBase` 民居 10→60、宅邸 80→400、工坊 40→200、商铺 50→300（各级同比例）。旧税基与物价完全脱节：一栋商铺月税 50 文，而它雇一个店员就要付 1500 文/月——税基只有人工成本的 3%。
- **铸币局 `mintPerWorkerDay` 50→800**（P0）：旧值是**确定的亏本项目**（4 岗 × 50 文/日 × 3 日 = 600 文/月收入，对上 4 × 1200 = 4800 文/月工资，净亏 4200）。新值下月铸 9600 文、净赚 4800，6 万造价约 12.5 游戏月回本。
- **售价按鲜度折算**（P1）：新增 `Goods.FreshPriceFactor`（下限 `FreshPriceFloor = 0.5`，坏了也有残值），接入 `RetailPrice`（取该铺该货的加权平均鲜度）与 `BuyerPrice`（取**被交易那一批**的实际鲜度，收货方按质论价）。堵住"把快失效的货按新货价卖给 NPC 铺面"的无损转嫁——此前铺面按基价照收，整套时效机制形同虚设。
- **加工输入按鲜度折算**（P1）：`CraftingSystem` 的可产数（`byInputs`）与扣料都改走有效量（`EffectiveAmountOf` / `ConsumeEffective`）——鲜度 50% 的粮要耗双份才酿得出一份酒，湿柴要多烧。堵住"故意把粮放陈再加工"的套利。
- **农田产量与工资**（配套）：`yieldPerWorker` 50→150、`salary` 800→400。旧值下一个农夫月产粮值 167 文却领 800 文工资（官库补贴 4.8 倍）；新值下产出约 500 文/月对工资 400，农田接近自负盈亏，一块田约养活 16 人。
- 影响文件：`sim/Goods.cs`、`configs/EconomyConfig.cs`、`policy/TaxSystem.cs`、`sim/CraftingSystem.cs`、`agents/CitizenAgent.cs`、`ui/InspectPanel.cs`、`data/buildings.json`。
- **未做**（已记入 `ECONOMY.md` §9.1）：搬运量 `LoadUnits` 未随新锚上调（新消耗下一担只够 5 天，村民搬运频次会升约 4 倍，且它牵动背包/堆容量与砍树产出换算）；收入端未随物价调整（三人户食物支出仅占双职工收入 7%，若要食物成为真正压力需连同建筑造价全盘重定）。

---

## 批次九十五（2026-09-17）物品时效系统（第一批）：双时间轴内核 + 货品属性分批存储

需求：为所有物品引入状态系统并把房屋、道路一并纳入、复用同一套命名与机制；定义新鲜度（软轴，三阶段）与有效期（硬轴）两种阈值，阈值可被外界环境影响并支持部分回滚（露天与仓库不同，回锅/烘干能恢复但加速衰败）；游戏内物品按属性分批存储，不再简单数量叠加。

分四批实施，本批次为**批次①（内核 + 货品接入）**。

- **双时间轴模型**：`TimedState` 只存三个客观量——`Fresh`（软轴 0~100）/ `UsedLifespan`（硬轴已耗天数）/ `RenewCount`（翻新次数）。`Span` 与衰减速率**全部现算**（`TimedRules.Resolve`），因为环境会实时改变阈值；存派生量会在环境变化时失真，且被迫在每次季节切换时全城改写。`剩余 = max(0, Span - UsedLifespan)` 必须夹紧。
- **软轴三阶段**：全效果（≥60，折算 1.0）→ 渐变衰减（15~60，1.0→0.25 线性）→ 已失效（<15，0.25→0，"可作他用"）。效果打折统一落地为**消耗时有效量折算** `有效量 = 取用量 × EffectOf(Fresh)`：鲜度 50% 的柴只能当半份烧、陈粮要吃双倍才饱。折算系数 <0.02 的批次不参与正常消耗（防除零放大取用量）。
- **环境修正走可注册接口**（用户要求"单独的接口控制，包括但不限于露天室内季节、存储方式"）：`ITimedModifier` + `TimedRates` + `TimedRegistry`，各源乘算累积、中性基准为"露天 + 春季"。内置三源——位置（有顶室内 ×0.45 速率 / ×1.6 寿限、无顶棚屋、随身）、季节（夏 ×1.6 / 秋 ×0.85 / 冬 ×0.55）、临水（×1.35 速率 / ×0.9 寿限）。新增地窖、熏房、冰鉴只需实现一个类 + 一行注册。
- **物品属性分批存储**（替代原来的"同货无条件并一堆"）：`SameBand` 判定 = 鲜度档（10 点/档）+ **已耗寿限档**（30 日/档）+ 翻新次数全同。刻意用"已耗"而非"剩余"分档（剩余是派生量，会随搬家而变，当键会导致同批货因环境变化被拆开）；合并键**不含环境标签**——衰减速率只取决于当前储存点与季节，同储存点内共享同一套倍率，故"露天坏得快、仓库坏得慢"会自然沉淀到各自 `Fresh` 数值里，两天后自动落档分开，无需环境标记、也无需季节变化时重新分批。同档内按份数加权平均。
- **最差优先取出**：`WorstIndex` 恒取同货品鲜度最低的批次（先吃快坏的）；`TakeBatch` → `StoreBatch` 成对新增，搬运时把时效状态一并带过去。
- **搬运保状态**（否则"从露天搬进仓库"这条核心玩法会在半路丢失损耗）：改造 `GameState.PickupPile` / `ClaimCellForBuilding`（扩地并入）、`CitizenAgent.Hauling`（背包入仓）与 `SellPack`（卖货给铺面）、`Stall.TickTrade`（货郎售货）、`VisitorSystem` 进口入库。生产端（收获、加工、开局赠送）仍走全新状态，无需改动。
- **衰减器**：`TimelinessSystem.TickDay` 遍历三类主循环持有者（建筑仓 / 村民背包 / 地面堆），计龄与双轴推进在同一次遍历完成（原 `GoodsSystem.AgeAllInventories` 已并入）；硬轴耗尽按 0.3 比例降级为 `Goods.Scrap`（已是燃料类货品，天然可作他用），或直接移除。访客背包由 `VisitorSystem.TickDay` 调 `AdvanceInventory`（随身携带口径）。
- **翻新模型（带代价的恢复）**：每次翻新恢复鲜度，但 `寿限 ×0.65^n`、`衰减速率 ×1.8^n` 复合叠加——递减收益自然涌现，无需额外规则禁止无限回锅。动作表与 UI 入口留待批次④。
- **时间尺度校准**：1 游戏日 = 20 秒现实（1 旬 = 3 日 = 60 秒、1 年 = 36 分钟），故阈值按游戏日给定——野味 10 日 / 烧饼 15 日 / 粮食 120 日（硬轴 480 日）/ 柴薪 240 日（无硬轴）。**未登记基线的货品一律不参与时效**（矿石、书籍、废料），比逐项写"永不腐坏"更省事，也避免新增货品忘记登记导致莫名腐烂。
- **面板**：`InspectPanel` 储存区与地面堆逐批显示「鲜 78% / 渐损 45% / 已失效」+ 剩余可存天数 + 翻新次数；剩余天数按该库存**当前所处环境**现算，玩家把货搬进仓房能立刻看到数字变长（阈值回滚的直接体现）。
...
## 批次九十四（2026-09-17）经济循环整理：定价三出口（库存联动接线）+ 商税补面

需求：整理经济循环流程；随之闭环上一版梳理出的三项缺口。

- **库存联动定价接线**（此前 `Goods.StockPriceFactor` 阈值与档位备好却**全项目零调用**，铺面售价恒等于「基价 × 固定倍率」）：新增 `Goods.FillRateOf`（整仓占用率 `Inv.Total / Inv.Capacity`）与两个统一出口——`Goods.RetailPrice`（基价 × 1.5 × 库存倍率，家庭零买自用）、`Goods.BuyerPrice`（基价 × 库存倍率，铺面/工坊的买方议价：生产性补料与向居民收货同价）。倍率档位 ≥95% ×0.7、≥80% ×0.9、≤20% ×1.1、其余 ×1.0。
- **同货不同价消除**：家庭自动购粮（`GoodsSystem.BuyGoods`）与带单采买（`CitizenAgent.Shopping`）此前分别是 ×1.5 与 ×1.0。现按交易性质分两档——家庭补货取 `RetailPrice`（两条路径同价），工坊补料取 `BuyerPrice`。`BuyGoods` 改为逐铺现算单价、逐铺扣钱（各卖家库存不同价，无法再开场估一个统一可买量）。
- **商税补面**：新增 `GameState.PayFromBuildingTaxed`（买方为建筑的代扣结算，恒等式「买方出资 = 卖方所得 + 官库税收」，买方付不足时按同比例缩放、卖方不倒贴），接入「工坊成品运交商铺寄卖」（`CitizenAgent.Hauling`）与「铺面向居民收货」（`CitizenAgent.SellPack`）两处此前完全不征的环节。朝廷采购与外贸进出口不征。
- **顺带修泄漏**：`BuyGoods` 售货款按 `pay / 人数` 逐人平摊致除不尽余数凭空消失 → 改由末位雇工收余数；定价由 `long` 截断改四舍五入，低价货的溢价档不再被抹平（基价 > 0 时保底 1 文）。
- **面板可见**：`InspectPanel` 储存区显示占用率与定价档位（平价 / 偏高折价 / 满仓折价 / 缺货溢价），生产需求区区分「收价」与「零售」，免得玩家看到售价浮动却找不到缘由。
- **文档**：`specs/ECONOMY.md` 由设计期需求稿重写为代码对齐版（10 节）；本批次补 §4 定价三出口与商税两征法、§9 闭环记录与仍待办、§10 实际基价表（并标注设计锚点与实现值不平）。
- 影响文件：`sim/Goods.cs`、`sim/GoodsSystem.cs`、`core/GameState.cs`、`agents/CitizenAgent.cs`、`ui/InspectPanel.cs`。存档无新字段，无需迁移。

---

## 批次九十三（2026-09-01）水系/山体重构（汇流求解）+ 主峰与湖泊 + 渲染图层拆分

需求：河流更自然、出图处是宽大水面而非小点、随机 3~6 座大小不一湖泊、1~2 座极高主峰、除西北外点缀小山、地形与建筑拆分成不同图层。

- **水系脱离草图改汇流求解**：新增 `FlowRouter`（Priority-Flood 填洼 + D8 流向 + 逆序汇流累积，路由格 2m）与 `RiverNetwork`（汇流面积阈值选源 → 顺 D8 走线 → 河宽 ∝√面积（Hack 定律，5~34m）→ Chaikin 圆角 ×2 → 蛇曲（振幅 ∝河宽 + 谷底吸附）→ 河口外推 56m 出图 + 末段 170m 展宽 2.1×——治「出图是小点」）。`WaterConfig` 整体重写为汇流参数制。
- **湖泊三通道**：新增 `LakeGenerator`——天然洼地塘（Depression 连通块漫灌）+ 三档选址湖（大 70~110m / 中 32~58m / 小 12~26m，最低前沿洪水填充贴地形生长），共 3~6 座、总水面 5.5% 封顶。旧版两道闸门（`LakeMaxSiteLevel`/`LakeFloodTolerance`）导致从不生成的根因废除。
- **水位/河床**：`SolveLevels` 按洪泛拓扑序单趟取 max，水位沿流向单调不增（支流自动回灌，治汇流处跳变）；`CarveBed` 按连通水体最大离岸距离在 1.0~3.5m 间插值深度（窄河浅槽、大湖深盆）。
- **山体**：`MaxTerrainHeight` 64→110、`MinTerrainHeight` −3→−5；新增 2 座 88~105m 主峰（角向谐波高斯锥破对称 + 侵蚀后 `ReapplyPrimaryPeaks` 余弦羽化补削顶）；低矮小山 5~9→12~20 座成簇撒布，选址判据从「东南象限」改为「远离峰群影响」（东北/西南/中部皆可落），加坡角 ≤16° 与高宽比 ≤0.40 两道守卫。`WorldSketch` 保留山形职责 + 预览河网（同源低分辨率骨架）。
- **渲染图层拆分**：`GridRenderer`（1338 行）瘦身为协调器（~280 行），内容拆成 `scripts/map/layers/` 下六图层——`TerrainLayer`/`WaterLayer`/`RoadLayer`/`VegetationLayer`/`BuildingLayer`/`OverlayLayer`（基类 `MapLayer`）+ `ChunkGeometryBuilder`（单趟遍历产出全组缓冲，免四层各扫一遍）+ `LayerKit`/`ChunkGeometry`（共用工具与缓冲容器）。脏标记与每帧预算仲裁（12 块/帧 + 32 块树）集中在协调器；对外 API `SetGridVisible`/`SetZonesVisible` 不变；裙板留协调器（属 Scroll 层）。地形顶点色迁入 TerrainLayer 并新增雪线（68m 起）适配主峰。
- 复核：`BuildController` 拾取下界（−5.5 低于最深湖床 −4.95，无需改）；`HeightField.ToBlob` 自适应量化确认抬上限不破存档。
